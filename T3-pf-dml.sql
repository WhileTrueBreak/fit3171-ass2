...
BEGIN
	...
	SELECT
		V.VISIT_ID INTO VISIT_ID
	FROM
		VISIT V
	WHERE
		V.FROM_VISIT_ID = FROM_VISIT_ID
		AND ROWNUM = 1;
	DBMS_OUTPUT.PUT_LINE(VISIT_ID);
	DBMS_OUTPUT.PUT_LINE(FROM_VISIT_ID);
	delete from VISIT_SERVICE v where v.VISIT_ID = VISIT_ID;
--  DELETE FROM VISIT_DRUG VD
--  WHERE
--  	VD.VISIT_ID = VISIT_ID;
--  DELETE FROM VISIT_SERVICE VS
--  WHERE
--  	VS.VISIT_ID = VISIT_ID;
--  DELETE FROM VISIT V
--  WHERE
--  	V.FROM_VISIT_ID = FROM_VISIT_ID;
END;
...
delete from VISIT_SERVICE v where v.VISIT_ID = 110;
select * from VISIT_SERVICE v;
